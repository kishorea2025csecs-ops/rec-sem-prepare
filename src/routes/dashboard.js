...
import { LogOut, User, Settings as SettingsIcon, ChevronDown, BookOpen, Layout, Upload, FileText, TrendingUp, CheckCircle2, AlertCircle, Youtube, ArrowRight, Plus, Search, ChevronRight, Clock, Sparkles, History, Zap, Image as ImageIcon, MessageSquare, BarChart3, Loader2, Wrench, Rocket, PlusCircle, ShieldCheck, Share2, Calendar, Star, } from "lucide-react";
...
function Dashboard() {
    ...
    const [explanationLevel, setExplanationLevel] = useState("quick");
    useEffect(() => {
    ...
    return (<div className="flex min-h-screen bg-background text-foreground">
      ...
      <main className="flex-1 md:pl-64">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border/60 bg-background/80 px-6 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-3 rounded-full border border-border bg-surface px-3 py-1.5 transition-colors hover:bg-surface/80">
                 {profile?.avatar_url || user?.user_metadata?.avatar_url ? (<img src={profile?.avatar_url || user?.user_metadata?.avatar_url} alt="Avatar" className="size-6 rounded-full object-cover"/>) : (<div className="size-6 rounded-full bg-gradient-to-tr from-accent to-primary"/>)}
              ...
              {showUserMenu && (<div className="absolute right-0 mt-2 w-48 overflow-hidden rounded-2xl border border-border bg-popover shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className="p-1.5">
                    <button onClick={() => navigate({ to: "/profile" })} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-medium hover:bg-surface transition-colors">
                    ...
                    <button onClick={handleLogout} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-medium text-destructive hover:bg-destructive/10 transition-colors">
          ...
          <div className="flex flex-col gap-8">
        ...
        ].map((stat) => (<div key={stat.label} className="rounded-2xl border border-border bg-card p-4 hover:border-primary/30 transition-colors group">
              ...
              <div className="bg-surface/50 px-6 py-4 border-b border-border flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="w-24 h-1.5 rounded-full bg-surface overflow-hidden">
                    <div className="h-full bg-primary transition-all duration-500" style={{ width: `${(activeLessonStep / 6) * 100}%` }}/>
                  </div>
                </div>
              </div>
              <div className="p-6">
            ...
            { title: "Share Your Work", icon: Share2, desc: "7: Share Your Work – Show your results to others.", color: "from-pink-500/20 to-pink-600/20", img: "https://cdn.iconscout.com/icon/premium/png-512-thumb/sharing-2601725-2184143.png?f=webp&w=256" },
        ].map((step, i) => (<button key={i} onClick={() => setActiveLessonStep(i)} className={`group relative flex flex-col items-center text-center p-3 rounded-2xl transition-all ${activeLessonStep === i
                ...
                : "hover:bg-surface/50 opacity-60 hover:opacity-100"}`}>
                      <div className={`relative flex size-14 items-center justify-center rounded-xl bg-gradient-to-br ${step.color} border border-white/5 overflow-hidden transition-transform duration-300 group-hover:scale-105`}>
                        <img src={step.img} alt={step.title} className="size-10 object-contain transition-transform duration-500 group-hover:rotate-6 group-hover:scale-110"/>
                        <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity"/>
                        
                        {activeLessonStep > i && (<div className="absolute -top-1 -right-1 size-4 bg-emerald-500 rounded-full flex items-center justify-center border-2 border-background shadow-sm">
                      ...
                      {activeLessonStep === i && (<div className="mt-2 absolute -bottom-20 left-1/2 -translate-x-1/2 w-48 z-20 pointer-events-none">
            ...
            <div className="border-b border-border">
              <nav className="-mb-px flex gap-8">
                {["Preparation Plan", "Unit Notes", "Previous Papers", "Tamil Tutorials"].map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className={`pb-4 text-sm font-medium transition-colors relative ${activeTab === tab ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}>
                    {tab}
              ...
              <div className="lg:col-span-2">
                {activeTab === "Preparation Plan" && (<>
                    ...
                    <div className="mt-6 space-y-4">
                      {topics.map((topic) => (<div key={topic.id} className="group relative overflow-hidden rounded-2xl border border-border bg-card transition-all hover:border-primary/50">
                            ...
                            <div className="flex gap-4">
                              ...
                              <div>
                                ...
                                <div className="mt-2 flex items-center gap-3">
                                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${topic.importance === 'High' ? 'bg-red-500/10 text-red-500 border border-red-500/20' :
                    topic.importance === 'Medium' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' :
                        'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'}`}>
                            ...
                            <button onClick={() => setSelectedQuestion(topic)} className="inline-flex items-center gap-2 rounded-xl bg-surface px-4 py-2 text-xs font-bold border border-border hover:bg-primary hover:text-primary-foreground transition-all">
                              Guide <ArrowRight className="size-3"/>
                            </button>
                          </div>
                          
                          {selectedQuestion?.id === topic.id && (<div className="border-t border-border bg-surface/30 p-6 animate-in fade-in slide-in-from-top-4 duration-300">
                        ...
                        { l: "Applications", m: "2m" },
                    ].map(item => (<div key={item.l} className="flex items-center justify-between text-[10px]">
                                  ...
                                  <div>
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="flex gap-1">
                                        {["quick", "exam", "revision"].map((l) => (<button key={l} onClick={() => setExplanationLevel(l)} className={`rounded-lg px-2 py-1 text-[9px] font-bold uppercase tracking-widest border transition-all ${explanationLevel === l ? 'bg-primary border-primary text-primary-foreground' : 'bg-surface border-border text-muted-foreground'}`}>
                                            {l}
                                          </button>))}
                                      </div>
                                    </div>
                                    ...
                                    <div className="mt-4 rounded-xl border border-border bg-card p-5">
                                      {explanationLevel === 'quick' && (<div className="animate-in fade-in duration-300">
                                      ...
                                      {explanationLevel === 'exam' && (<div className="animate-in fade-in duration-300 space-y-3">
                                      ...
                                      {explanationLevel === 'revision' && (<div className="animate-in fade-in duration-300">
                                          <ul className="space-y-2">
                                            {["DAG", "Node Dependencies", "CPT Tables"].map(kw => (<li key={kw} className="flex items-center gap-2 text-[10px] font-bold">
                ...
                {activeTab === "Unit Notes" && (<div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <h2 className="font-display text-xl font-bold">Unit Analysis</h2>
                      {analysisStatus === "idle" && (<button onClick={startAnalysis} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-primary-foreground shadow-sm hover:-translate-y-0.5 transition-all">
                          <Sparkles className="size-3.5"/> Analyze Notes with AI
                        </button>)}
                    </div>

                    {analysisStatus !== "idle" && analysisStatus !== "complete" && (<div className="rounded-3xl border border-border bg-card p-8 text-center">
                        ...
                        <div className="mt-8 h-2 w-full max-w-md mx-auto overflow-hidden rounded-full bg-surface">
                          <div className="h-full bg-primary transition-all duration-300 ease-out" style={{ width: `${processingProgress}%` }}/>
                        </div>
                      </div>)}

                    {analysisStatus === "complete" && (<div className="grid gap-6">
                        ...
                        <div className="grid gap-6 sm:grid-cols-3">
                          {extractedConcepts.map((concept) => (<div key={concept.category} className="rounded-2xl border border-border bg-card p-5">
                              ...
                              <ul className="space-y-3">
                                {concept.items.map((item, idx) => (<li key={idx} className="text-sm text-muted-foreground flex items-start gap-2">
                    ...
                    {analysisStatus === "idle" && (<div className="rounded-3xl border-2 border-dashed border-border p-12 text-center bg-surface/30">
                        ...
                        <button onClick={startAnalysis} className="mt-6 inline-flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-2 text-sm font-semibold hover:bg-surface transition-colors">
                          Select PDF Files
                        </button>
                      </div>)}
                  </div>)}

                {activeTab === "Previous Papers" && (<div className="space-y-6">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="grid gap-4">
                      {pyqData.map((item, idx) => (<div key={idx} className="rounded-2xl border border-border bg-card p-6 transition-all hover:border-accent/50 group">
                          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                            <div className="space-y-4 flex-1">
                              <div className="flex items-center gap-2">
                                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${item.importance === 'Very High' ? 'bg-red-500/15 text-red-500' : 'bg-accent/15 text-accent'}`}>
              ...
              <div className="space-y-6">
                <div className="rounded-3xl border border-border bg-card overflow-hidden">
                  ...
                  <div className="p-5 space-y-4">
                    <div onClick={() => setActiveTab("Unit Notes")} className={`rounded-2xl border p-4 transition-all cursor-pointer ${activeTab === "Unit Notes" ? "bg-primary/5 border-primary/30" : "border-border hover:bg-surface/50"}`}>
                      <div className="flex items-center gap-3">
                    ...
                    <div onClick={() => setActiveTab("Previous Papers")} className={`rounded-2xl border p-4 transition-all cursor-pointer ${activeTab === "Previous Papers" ? "bg-accent/5 border-accent/30" : "border-border hover:bg-surface/50"}`}>
                      <div className="flex items-center gap-3">
                    ...
                    <div className="mt-4 flex flex-col gap-2">
                      <button onClick={startAnalysis} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-2.5 text-xs font-bold text-primary-foreground shadow-sm hover:-translate-y-0.5 transition-transform">
                        <Upload className="size-3.5"/> Upload New Notes
                      </button>
                      <button onClick={() => setActiveTab("Tamil Tutorials")} className="flex w-full items-center justify-center gap-2 rounded-xl border border-border py-2.5 text-xs font-bold hover:bg-surface transition-colors">
                ...
                <div className="rounded-3xl border border-border bg-card overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="aspect-square w-full relative rounded-2xl bg-surface/30 overflow-hidden group cursor-crosshair border border-border">
                      <iframe src="https://my.spline.design/widgetscarouselcopycopy-cfc7yNWcEtRgYFIXmHzEZWoV-JSW/" frameBorder="0" width="100%" height="100%" className="h-full w-full pointer-events-none opacity-80 transition-transform duration-500 group-hover:scale-110" title="3D Design Analysis"/>
